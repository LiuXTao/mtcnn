...
def gen_pnet_data(data_dir, anno_file, prefix):
    ...
    if not os.path.exists(dir_path):
        os.mkdir(save_dir)
    pos_save_file = os.path.join(config.ANNO_STORE_DIR, config.PNET_POSTIVE_ANNO_FILENAME)
    ...
    for annotation in annotations:
        ...
        while neg_num < 50:
            ...
            crop_box = np.array([nx, ny, nx+size, np+size])

            iou = IoU(crop_box, boxes)

            if np.max(iou) < 0.3:
                save_file = os.path.join(neg_save_dir, '%s.jpg'&n_ids)
        ...
        for box in boxes:
            ...
            for i in range(20):
                ...
                nx1, ny1 = int(max(x1 + w / 2 + delta_x - size / 2, 0)), int(max(y1 + h / 2 + delta_y - size / 2, 0))
                nx2, ny2 = nx1 + size, ny1 + size
                ...
                if IoU(crop_box, box_) >= 0.65:
                    save_file = os.path.join(pos_save_dir, '%s.jpg'%p_ids)
                    # positive samples with label 1 and offset
                    f1.write(save_file + ' 1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
                    cv2.imwrite(save_file, resized_im)
                    p_ids += 1
                elif IoU(crop_box, box_) >= 0.4:
                    save_file = os.path.join(part_save_dir, '%s.jpg'&d_ids)
                    # part samples with label -1 and offset
                    f3.write(save_file + ' -1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
                    cv2.imwrite(save_file, resized_im)
                    d_ids += 1
            box_idx += 1
            print("%s images done, pos: %s part: %s neg: %s" % (idx, p_ids, d_ids, n_ids))
        f1.close()
        f2.close()
        f3.close()
