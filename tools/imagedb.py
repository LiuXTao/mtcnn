...
class ImageDB(object):
    ...
    def load_annotation(self, annotation_type=1):
        ...
        for i in range(self.num_images):
            ...
            if self.mode == 'test':
                ...
                if len(annotation[2:]) == 14:
                    ...
                    imdb_['bbox_target'] = np.array(landmark).astype(float)
            imdb.append(imdb_)
    ...
    def append_flipped_images(self, imdb):
        ...
        for i in range(length):
            ...
            landmark_ = np.array([(1 - x), y] for (x, y) in landmark_)
            landmark_[[0, 1]] = landmark_[[1, 0]]
            landmark_[[3, 4]] = landmark_[[4, 3]]

            item = {'image': imdb_['image'], 'label': imdb_['label'], 'bbox_target': m_bbox, 'landmark_target': landmark_.reshape((10)), 'flipped': True}

            imdb.append(item)

        self.image_set_index *= 2

        print('after flipped images to imdb', len(imdb))
