...
def train_pnet(anno_file, model_store, end_epoch, frequent, base_lr, batch_size, use_cuda=False):
    if not os.path.exists(model_store):
        os.makedirs(model_store)

    pnet = PNet(is_train=True, use_cuda=use_cuda)
    ...
    optimizer = optim.Adam(pnet.parameters(), lr=base_lr)
    ...
    for cur_epoch in range(end_epoch):
        ...
        for batch_idx, (image, (gt_label, gt_bbox, gt_landmark)) in enumerate(train_data):
            ...
            cls_pred, box_offset_pred = pnet(im_tensor)
            cls_loss = loss_function.cls_loss(gt_label, cls_pred)
            box_loss = loss_function.box_loss(gt_label, gt_bbox, box_offset_pred)
            all_loss = cls_loss *1.0 + box_loss *0.5
            if batch_idx % frequent ==0:
                ...
                print(
                    ...
                    (datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), cur_epoch + 1, batch_idx,
                     ...
                     all_loss.data.tolist(), cls_loss.data.tolist(), box_offset_pred.data.tolist(),
                     scheduler.get_lr()[0]))
            optimizer.zero_grad()
            all_loss.backward()
            optimizer.step()
        ...
        if (cur_epoch + 1) % 10 ==0:
            torch.save(pnet.state_dict(), os.path.join(model_store, "pnet_model_epoch_%d.pt"%(cur_epoch+1)))
    torch.save(pnet.state_dict(), os.path.join(model_store, 'pnet_model_final.pt'))
...
if __name__ == '__main__':
    ...
    train_pnet(anno_file, model_store, end_epoch, frequent, base_lr, batch_size)