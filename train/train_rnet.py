...
def train_rnet(anno_file, model_store, end_epoch, frequent, base_lr, batch_size, use_cuda=False):
    if not os.path.exists(model_store):
        os.makedirs(model_store)

    rnet = RNet(is_train=True, use_cuda=use_cuda)
    ...
    optimizer = optim.Adam(rnet.parameters(), lr=base_lr)
    ...
    for cur_epoch in range(end_epoch):
        ...
        if batch_idx % 10 == 0:
            torch.save(rnet.state_dict(), os.path.join(model_store, 'rnet_model_epoch_%d.pt'%(batch_idx + 1)))
    torch.save(rnet.state_dict(), os.path.join(model_store, 'rnet_model_final.pt'))
...
if __name__ == '__main__':
    ...
    train_rnet(anno_file, model_store, end_epoch, frequent, base_lr, batch_size)
